...
local function update_application(server_name_prefix, data)
  local sn
  local up_apps = ngx.table.new(0, #data)
  ...
  for _, service in ipairs(data) do
    ...
    if service.port == 0 then
      log.error("no valid service port can be found, service: ", service.port)
      goto CONTINUE
    end
...
function _M.connect(premature, consul_server)
  ...
  if consul_server.index ~= result.headers['X-Consul-Index'] then
    ...
    if result.body and #result.body ~= 0 then
      log.notice("consul server: ", consul_server.server_name_key, ", header: ",
                 core.json.encode(result.headers, true), ", body: ",
                 core.json.encode(result.body, true))
...
function _M.nodes(service_name)
  if not applications then
    log.error("applications is nil, failed to fetch nodes for: ", service_name)
    return
  end
